...
export default function PerformancePage() {
  ...
  const { chartData, yAxisDomain, chartConfig, legendUsers, currentWk } = useMemo(() => {
    ...
    // Determine currentWk from the available scores in histories
    const allWks = activeHistories.flatMap(h => h.weeklyScores.map(ws => Number(ws.week)));
    const maxWk = allWks.length > 0 ? Math.max(...allWks) : 0;

    const allScores = activeHistories.flatMap(h => h.weeklyScores.map(w => w.score));
    if (!allScores.length) return { chartData: [], yAxisDomain: [0, 10], chartConfig: {}, legendUsers: [], currentWk: maxWk };
  ...
  return (
    <div className="space-y-8">
      {isLoading ? (
          ...
          <Card>
            <CardHeader><CardTitle>Player Scores By Week</CardTitle><CardDescription>Weekly score progression up to Week {currentWk}.</CardDescription></CardHeader>
            <CardContent>
              ...
              <PlayerPerformanceChart chartData={chartData} yAxisDomain={yAxisDomain} sortedUsers={activeUsers} chartConfig={chartConfig} currentUserId={resolvedUserId} />
            </CardContent>
          </Card>
      )}
    </div>
