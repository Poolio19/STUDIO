...
export default function PerformancePage() {
  ...
  const { chartData, yAxisDomain, chartConfig, legendUsers } = useMemo(() => {
    ...
    // Filter score values strictly by chronological week for the Y-Axis domain
    const allChronologicalScores = activeUserHistories.flatMap(h => 
        h.weeklyScores.filter(w => w.week >= 0 && w.week <= currentChronologicalWeek).map(w => w.score)
    );

    if (allChronologicalScores.length === 0) return { chartData: [], yAxisDomain: [0, 10], chartConfig: {}, legendUsers: [] };
    
    const minScore = Math.min(...allChronologicalScores);
    const maxScore = Math.max(...allChronologicalScores);
      ...
      {isLoading ? (
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Each line represents a player's total score over the chronological weeks. Future games are excluded from the visual timeline.
              </CardDescription>
            </CardHeader>
            <CardContent>
